...
enum TokenCode {
    ...
    TOKEN_IDENTIFIER,     // 2 - (String token)
...
// Simbolos Especiais Compostos
    TOKEN_COLON_EQUAL,    // 19 - :=
    TOKEN_DOT_DOT,        // 20 - ..
    TOKEN_LPAREN_STAR,    // 21 - (*
    TOKEN_RPAREN_STAR,    // 22 - *)
...
const std::unordered_map<String, TokenCode> simbolos_especiais = {
        {".", TOKEN_DOT},   {":", TOKEN_COLON},   {",", TOKEN_COMMA},  {"(", TOKEN_LPAREN},
        {")", TOKEN_RPAREN}, {"=", TOKEN_EQUAL},   {"<", TOKEN_LT},     {">", TOKEN_GT},
        {"+", TOKEN_PLUS},   {"-", TOKEN_MINUS},   {"*", TOKEN_STAR},   {"[", TOKEN_LBRACKET},
        {"]", TOKEN_RBRACKET}, {":=", TOKEN_COLON_EQUAL}, {"..", TOKEN_DOT_DOT},
        {"(*", TOKEN_LPAREN_STAR}, {"*)", TOKEN_RPAREN_STAR}, {";", TOKEN_SEMICOLON}, {"'", TOKEN_APOSTROPHE}
};

const std::unordered_map<String, TokenCode> palavras_chave = {
        {"program", TOKEN_PROGRAM}, {"label", TOKEN_LABEL}, {"type", TOKEN_TYPE},
        {"array", TOKEN_ARRAY}, {"of", TOKEN_OF}, {"var", TOKEN_VAR}, {"procedure", TOKEN_PROCEDURE},
        {"function", TOKEN_FUNCTION}, {"begin", TOKEN_BEGIN}, {"end", TOKEN_END}, {"if", TOKEN_IF},
        {"then", TOKEN_THEN}, {"else", TOKEN_ELSE}, {"while", TOKEN_WHILE}, {"do", TOKEN_DO},
...
token analisador_lexico(std::string::iterator &prox,const std::string::iterator &end) {
    ...
    if(prox == end) return {"Fim de cadeia.", TOKEN_ERROR};
    ...
    if(islower(*prox)) {
        ...
        else return {atom, TOKEN_IDENTIFIER};
    }
